...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        # 4.调用 API 求出 son1 相对于 son2 的坐标关系
            ...
            rospy.loginfo("Son1 relative to Son2: ")
            rospy.loginfo("father ID: %s",tfs.header.frame_id)
            rospy.loginfo("child ID: %s",tfs.child_frame_id)
            rospy.loginfo("Son1 relative to Son2: x=%.2f, y=%.2f, z=%.2f",
        ...
        # 5.创建一依赖于 son1 的坐标点，调用 API 求出该点在 son2 中的坐标
            ...
            rospy.loginfo("point_target in %s coordinate system",point_target.header.frame_id)
            rospy.loginfo("the positon relative to son2:(%.2f,%.2f,%.2f)",
